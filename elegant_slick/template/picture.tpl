...
<div id="derivativeSwitchBox" class="switchBox">
  ...
  {/foreach}
  {if isset($U_ORIGINAL)}
  <a href="javascript:changeImgSrc('{$U_ORIGINAL}','Original','Original')">
    {'Original'|@translate}<span class="derivativeSizeDetails">
  </a>
  {/if}
</div>
{/strip}
...
{* TF, 20160409: add openstreetmap from category *}
{if !empty($OSMJS)}
<div id="map" class="maphidden"></div>
{combine_script id='my.mapscript' load='footer' require='jquery' path='local/js/my.mapscript.js'}
...
{if $DISPLAY_NAV_THUMB}
	<div class="navThumbs">
{* TF, 20160317: no title tags please since we use css cursor image *}
{* TF, 20160317: us linkUp id tag to match css *}
		{if isset($previous)}
			<a class="navThumb" id="linkPrev" href="{$previous.U_IMG}" rel="prev">
				<span class="thumbHover prevThumbHover"></span>
				<img src="{$previous.derivatives.square->get_url()}" alt="{$previous.TITLE_ESC}">
			</a>
		{elseif isset($U_UP)}
			<a class="navThumb" id="linkUp" href="{$U_UP}" >
				<div class="thumbHover">{'First Page'|@translate}<br><br>{'Go back to the album'|@translate}</div>
			</a>
		{/if}
		{if isset($next)}
			<a class="navThumb" id="linkNext" href="{$next.U_IMG}" rel="next">
...
{* TF, 20160402: add info from thumbnails *}
{include file="./picture_thumbnails_slick.tpl"}
