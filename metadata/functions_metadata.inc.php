...
function get_exif_data($filename, $map)
{
  ...
  if ($exif = @read_exif_data($filename) or $exif2 = trigger_change('format_exif_data', $exif=null, $filename, $map))
  {
    ...
    {
      if (
        is_array($gps_exif['GPSLatitude'])  and in_array($gps_exif['GPSLatitudeRef'], array('S', 'South', 'N', 'North')) and
        is_array($gps_exif['GPSLongitude']) and in_array($gps_exif['GPSLongitudeRef'], array('W', 'West', 'E', 'East'))
        )
      {
		...
		// TFE, 20170710: overwrite original GPSLatitude, GPSLongitude array values - otherwise below strip_tags will fail
        $result['GPSLatitude'] = $result['latitude'] . " " . $gps_exif['GPSLatitudeRef'];
        $result['GPSLongitude'] = $result['longitude'] . " " . $gps_exif['GPSLongitudeRef'];
      }
    }
  }
